...
import thefellas.safepoint.core.utils.ColorUtil;
...
public class NameTags extends Module {
    ...
    BooleanSetting dynamicOutline = new BooleanSetting("Dynamic Outline", false, this);
    FloatSetting outlineWidth = new FloatSetting("Outline Width", 1.0f, 1.0f, 3.0f, this, v -> outline.getValue());
    ColorSetting outlineColor = new ColorSetting("Outline Color", new Color(255, 0, 0, 255), this, v -> outline.getValue() && !dynamicOutline.getValue());
    FloatSetting yOffset = new FloatSetting("Y Offset", 0, -10.0F, 10.0F, this);
    FloatSetting sizel = new FloatSetting("Size", 1.3F, -10.0F, 10.0F, this);
    ...
    public void onRenderWorld(RenderWorldLastEvent event) {
        for (Entity e : NameTags.mc.world.loadedEntityList) {
            ...
            if(outline.getValue()){RenderUtil.drawOutlineRect((int)(-NameTags.mc.fontRenderer.getStringWidth(e.getName() + " " + health + "%") / 2 - 2) - width.getValue(), (int)-2 - height.getValue(), (int)(NameTags.mc.fontRenderer.getStringWidth(e.getName()) / 2 + 16) + width.getValue(), (int)10 + height.getValue(),(dynamicOutline.getValue() ? new Color(ColorUtil.getHealthColor(e)) :  outlineColor.getColor()), outlineWidth.getValue());}
            Safepoint.fontInitializer.draw(e.getName() + " " + (Object) TextFormatting.GREEN + health + (heart.getValue() ? "\u2764" : "%"), 0 - this.getcenter(e.getName() + " " + (Object) TextFormatting.GREEN + health + "%"), 1, -1);
                ...
                if (Item.getIdFromItem((Item) ((EntityPlayer) e).inventory.getCurrentItem().getItem()) != 0) {
                    ...
                    for (Object enchantment : enchantments.keySet()) {
                        if(enchant.getValue()) {
                            ...
                            Safepoint.fontInitializer.draw(String.valueOf(((Enchantment) enchantment).getName().substring(12).charAt(0)).toUpperCase() + level, (float) (posX + 6 - this.getcenter(String.valueOf(((Enchantment) enchantment).getName().substring(12).charAt(0)).toUpperCase() + level)), (float) posY, -1);
                            posY -= 12;
                        }
                    }
                    posX += 15;
                ...
                if(armor.getValue()) {
                    ...
                    for (Object enchantment : enchantments.keySet()) {
                        if(enchant.getValue()) {
                            ...
                            Safepoint.fontInitializer.draw(String.valueOf(((Enchantment) enchantment).getName().substring(12).charAt(0)).toUpperCase() + level, (float) (posX + 9 - this.getcenter(((Enchantment) enchantment).getName().substring(12).charAt(0) + level)), (float) posY, -1);
                            posY -= 12;
                        }
                    }
                    posX += 17;
            ...
            if (gapples > 0) {
                ...
                NameTags.mc.getRenderItem().zLevel = 0.0f;
                Safepoint.fontInitializer.draw(String.valueOf(gapples), (float) (posX + 9 - this.getcenter(String.valueOf(gapples))), -30.0f, -1);
            }
