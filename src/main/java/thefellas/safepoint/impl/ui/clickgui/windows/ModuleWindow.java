...
import thefellas.safepoint.impl.ui.clickgui.settingbutton.impl.*;
...
public class ModuleWindow {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (isInside(mouseX, mouseY)) {
            ...
            Safepoint.fontInitializer.draw(module.getDescription(), x, 3, -1);
        }
        Safepoint.fontInitializer.draw(name, isInside(mouseX, mouseY) ? x + 3 : x + 2, (y + (height / 2f) - (Safepoint.mc.fontRenderer.FONT_HEIGHT / 2f)), -1);
        if (module.isOpened) {
            int y = this.y;
