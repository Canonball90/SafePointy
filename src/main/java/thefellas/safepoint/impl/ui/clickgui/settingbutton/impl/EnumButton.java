...
public class EnumButton extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Safepoint.fontInitializer.draw(enumSetting.getName() + " " + ChatFormatting.GRAY + enumSetting.getValueAsString(), x + 2,(y + (height / 2f) - (Safepoint.mc.fontRenderer.FONT_HEIGHT / 2f)), -1);
        int y = this.y;
        if (enumSetting.droppedDown) {
            for (String string : enumSetting.getModes()) {
                ...
                Safepoint.fontInitializer.draw(enumSetting.getValue().equals(string) ? string : ChatFormatting.GRAY + string, (mouseX > x && mouseX < x + width && mouseY > y && mouseY < y + height) ? x + 5 : x + 4, y, -1);
            }
