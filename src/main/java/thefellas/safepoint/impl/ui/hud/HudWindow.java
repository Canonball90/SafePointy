...
public class HudWindow extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Safepoint.fontInitializer.draw("Hud Editor", (int) (x + (w / 2f) - (Safepoint.mc.fontRenderer.getStringWidth("Hud Editor") / 2f)), (int) (y + (h / 2f) - (Safepoint.mc.fontRenderer.FONT_HEIGHT / 2f)), -1);
        ...
        Safepoint.fontInitializer.draw("x", x + w - Safepoint.mc.fontRenderer.getStringWidth("x") - 2, (int) (y + (h / 2f) - (Safepoint.mc.fontRenderer.FONT_HEIGHT / 2f)), -1);
        ...
        if (isInsideCloseButton(mouseX, mouseY)) {
            ...
            Safepoint.fontInitializer.draw("Left Click: Close | Right Click: Return", mouseX + 5, mouseY + 1, -1);
        }
        int yIncrease = y;
    ...
    boolean isInsideCloseButton(int mouseX, int mouseY) {
        return (mouseX > x + w - (CustomFont.INSTANCE.isEnabled() ? Safepoint.customFontRenderer.getStringWidth("x") : mc.fontRenderer.getStringWidth("x")) - 4 && mouseX < x + w) && (mouseY > y && mouseY < y + h);
    }
