...
public class HudWelcomerComponent extends HudModule {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        if (getValue()) {
            if (isInsideDragField(mouseX, mouseY)) {
                ...
                Safepoint.fontInitializer.draw("renderX: " + renderX + " renderY: " + renderY, renderX + Safepoint.mc.fontRenderer.getStringWidth("P-Client 0.1") + 3, renderY - 7, -1);
            }
            drawText();
        }
    }
    ...
    public void drawText() {
        Safepoint.fontInitializer.draw(ChatFormatting.BOLD + "Welcome to Safepoint.club, " + ChatFormatting.RESET + Minecraft.getMinecraft().player.getName().toString() + ":)", renderX, renderY, AC_ClickGui.getInstance().color.getColor().getRGB());
    }
