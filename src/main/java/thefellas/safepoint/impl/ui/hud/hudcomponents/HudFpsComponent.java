...
public class HudFpsComponent extends HudModule {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        if (getValue()) {
            if (isInsideDragField(mouseX, mouseY)) {
                ...
                Safepoint.fontInitializer.draw("renderX: " + renderX + " renderY: " + renderY, renderX + Safepoint.mc.fontRenderer.getStringWidth("SafePoint.club") + 3, renderY - 7, -1);
            }
            drawText();
        }
    }
    ...
    public void drawText() {
        Safepoint.fontInitializer.draw("FPS: " + Minecraft.getDebugFPS(), renderX, renderY, AC_ClickGui.getInstance().color.getColor().getRGB());
    }
